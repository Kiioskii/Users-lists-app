...
if (isset($_GET['toCheck'])) {
    ...
    $listData = $connection->query("SELECT * FROM user_lists WHERE id='$id'");
    $row = mysqli_fetch_assoc($listData);

    $nazwa = $row['list_name'];
    echo ($nazwa);
    echo (";");

    $result = $connection->query("SELECT user_id FROM user_list_members WHERE list_id='$id'");
    $users = array();
...
if (isset($_POST['isEdited'])) {
    ...
    $list_id = $connection->real_escape_string($_POST['list_id']);
    $name = $connection->real_escape_string($_POST['name']);
    $ids = json_decode(stripslashes($_POST['ids']));

    $connection->query("UPDATE `user_lists` SET list_name='$name' WHERE id='$list_id'");
    $connection->query("DELETE  FROM  `user_list_members` WHERE list_id='$list_id'");
