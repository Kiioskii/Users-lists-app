...
if (isset($_GET['lists'])) {
    ...
    if (mysqli_num_rows($all_lists) > 0) { ?>
        <table class="users-table-container"><!--Displaying the table with set of lists  -->
            <thead>
                <tr>
                    <th>Nazwa</th><!--column that allows to view list name  -->
                    <th>Należy do listy</th><!--column that allows to view what lists the user belongs to   -->
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                ?>
                    <tr class="users-data-row" item="<?php echo $id ?>">
                        <td><?php echo $nazwa ?></td><!--column that allows to view list name  -->
                        <td><!--column that allows to view what lists the user belongs to   -->
...
if (isset($_GET['user'])) {
    ...
    if (mysqli_num_rows($all_users) > 0) { ?>
        <table class="users-table-container"><!--Displaying the table with data of user with $user_id  -->
            <thead>
                <tr>
                    <th>Nazwa</th>
                    <th>Hasło</th>
